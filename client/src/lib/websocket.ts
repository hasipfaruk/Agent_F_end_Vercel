...
class WebSocketClient {
  ...
  connect(useDirectConnection = false) {
    ...
    // Always use relative URLs - the proxy in vercel.json will handle routing
    if (isProduction) {
      console.log("Running in production environment, using polling");
      this.startPolling();
      return;
    }
  ...
  private startPolling(isProduction = false, backendHost = '') {
    ...
    // Always use relative URL - the proxy in vercel.json will handle routing
    this.apiBaseUrl = '/api';
    console.log("Using API base URL:", this.apiBaseUrl);
  ...
  private async pollAgents() {
    try {
      ...
      // Simplified fetch without CORS options - the proxy handles this
      const response = await fetch(`${this.apiBaseUrl}/agents`);
